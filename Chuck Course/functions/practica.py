...
def update():
    ...
    micursor=miconex.cursor()
    datos=minombre.get(), miapellido.get(), midire.get(), mipass.get(), textocomen.get(1.0, END)
    micursor.execute("UPDATE DATOSUSUARIOS SET NOMBRE_USUARIO=COALESCE(?,NOMBRE_USUARIO), APELLIDO=?, DIRECCION=?, CONTRASEÑA=?, COMENTARIOS=? " +
...
def version():
    messagebox.showinfo("gilazo","Deja de apretar boludeces")
...
ayudamenu.add_command(label='licencia', command=version)
ayudamenu.add_command(label='acerca de...',command=explosivo)
