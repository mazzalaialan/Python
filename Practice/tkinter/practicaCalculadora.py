...
def coma(c):
	...
	if reset_pantalla!=False:
		numeroPantalla.set('0'+c)
		reset_pantalla=False
	else:
		if c in numeroPantalla.get():
			None
		else:
			numeroPantalla.set(numeroPantalla.get() + c)
...
botonComa=Button(miFrame, text=",", width=3, command=lambda:coma("."))
...
botonb=Button(miFrame, text="borrar", width=6, command=lambda:borrando())
botonb.grid(row=6, column=4)
